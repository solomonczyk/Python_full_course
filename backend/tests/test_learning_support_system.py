...
class TestFoundationBlocks:
    ...
    def test_all_lessons_have_foundation(self):
        data = _load_json("lessons.json")
        for lesson in data:
            assert "foundation" in lesson, \
                f"Lesson '{lesson['id']}' missing foundation block"
            assert isinstance(lesson["foundation"], dict), \
                f"Lesson '{lesson['id']}' foundation is not a dict"

    def test_first_10_lessons_have_terms(self):
        data = _load_json("lessons.json")
        first_10 = [l for l in data if int(l["id"].split("-")[0]) == 1
                    and int(l["id"].split("-")[1]) <= 9]
        for lesson in first_10:
            assert "terms" in lesson["foundation"], \
                f"Lesson '{lesson['id']}' foundation missing 'terms'"
            assert len(lesson["foundation"]["terms"]) >= 1, \
                f"Lesson '{lesson['id']}' foundation has no terms"
